...
export class Frame extends SdkObject {
  ...
  async ariaSnapshot(progress: Progress, selector: string, options?: { mode?: 'expect' | 'ai' }): Promise<string> {
    // For AI mode, calculate the cumulative offset from this frame to the main frame
    // so that viewport positions are calculated relative to the main viewport
    let mainFrameViewport: { offsetX: number, offsetY: number, width: number, height: number } | undefined;

    if (options?.mode === 'ai') {
      mainFrameViewport = await this._getMainFrameViewportInfo(progress);
    }

    const snapshotOptions = mainFrameViewport ? { ...options, mainFrameViewport } : options;
    return await this._retryWithProgressIfNotConnected(progress, selector, true /* strict */, true /* performActionPreChecks */, handle => progress.race(handle.ariaSnapshot(snapshotOptions)));
  }

  /**
   * Calculates the cumulative offset from this frame to the main frame's viewport.
   * This is used to determine the true visibility of elements in child frames
   * relative to the main viewport (what the user actually sees).
   *
   * Returns { offsetX, offsetY, width, height } where:
   * - offsetX/offsetY: cumulative offset from this frame's origin to main frame's origin
   * - width/height: main frame's viewport dimensions
   */
  private async _getMainFrameViewportInfo(progress: Progress): Promise<{ offsetX: number, offsetY: number, width: number, height: number } | undefined> {
    ...
    let mainViewportHeight = 0;

    // Walk up the frame hierarchy, accumulating offsets
    let frame: Frame | null = this;
    while (frame?.parentFrame()) {
      try {
        const frameElement = await progress.race(frame.frameElement());
        if (frameElement) {
          const result = await progress.race(frameElement.evaluateInUtility(([injected, element]) => {
          ...
          if (result) {
            // Add the iframe's position in its parent frame to cumulative offset
            cumulativeOffsetX += result.left;
            cumulativeOffsetY += result.top;

            // If we've reached the main frame (parent has no parent), store viewport size
            if (!frame.parentFrame()?.parentFrame()) {
              mainViewportWidth = result.viewportWidth;
              mainViewportHeight = result.viewportHeight;
            }
          }
        }
      } catch {
    ...
    // If we couldn't determine viewport size, return undefined
    if (!mainViewportWidth || !mainViewportHeight)
      return undefined;
